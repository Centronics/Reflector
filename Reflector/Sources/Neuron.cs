...
namespace DynamicReflector
{
    ...
    public sealed class Neuron
    {
        ...
        /// Для успешного выполнения всех запросов необходимо то, чтобы успешно отработали все запросы.
        ...
        /// Названия <see cref="Processor.Tag"/> берутся из запросов и полностью соответствуют им.
        /// В случае нахождения дрйгой эквивалентной карты по данному запросу, её переименование производится автоматически.
        /// После вызовать этого метода текущий экземпляр класса <see cref="Neuron"/> остаётся в том же состоянии, в котором он был до этого вызова.
        ...
        /// <returns>Возвращает новый <see cref="Neuron"/>, содержащий новые карты, взятые из результатов выполнения запросов.</returns>
